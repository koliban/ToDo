...
@SuppressLint("HandlerLeak") public class ActivityRecord extends AppCompatActivity {
	...
	class ClickEvent implements OnClickListener {
		...
		public void onClick(View v) {
			int i = v.getId();
			if (i == R.id.btn_record) {// 开始录音
				if (isRecording == 0) {
					// 每一次调用录音，可以录音多次，满意为至，最后只将最后一次的录音文件保存，其他的删除
					if (FilePath != null) {
				...
				// 点击保存按钮
			} else if (i == R.id.bt_save) {// 将最终的录音文件的路径返回到新增日记页面
				if (FilePath == null) {
			...
			} else if (i == R.id.bt_back) {// 返回前将录音的文件删除
				if (FilePath != null) {
					File oldFile = new File(FilePath);
					oldFile.delete();
				}
